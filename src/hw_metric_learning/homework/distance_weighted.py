...
def train_one_epoch(model, dataloader, optimizer, device, margin=0.5):
    model.train()
    running_loss = 0.0
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Объединяем все изображения и метки в один батч
        images = torch.cat([anchor, positive, negative], dim=0).to(device)
        labels = torch.cat([anchor_label, anchor_label, anchor_label], dim=0).to(device)

        optimizer.zero_grad()
        embeddings = model(images)

        mined_triplets = miner(embeddings, labels)
        loss = criterion(embeddings, labels, mined_triplets)
