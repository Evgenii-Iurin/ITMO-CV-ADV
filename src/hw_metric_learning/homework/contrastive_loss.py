...
def contrastive_loss(out1, out2, label, margin=1.0):
    # label == 1 → positive pair; label == 0 → negative pair
    euclidean_distance = F.pairwise_distance(out1, out2)
    loss = label * (euclidean_distance ** 2) + \
...
def train_one_epoch(model, dataloader, optimizer, device, margin=1.0):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Метки: 1 для positive пар, 0 для negative пар
        pos_labels = torch.ones(anchor_out.size(0)).to(device)
        neg_labels = torch.zeros(anchor_out.size(0)).to(device)

        # Contrastive loss для каждой пары
        pos_loss = contrastive_loss(anchor_out, positive_out, pos_labels, margin)
        neg_loss = contrastive_loss(anchor_out, negative_out, neg_labels, margin)
